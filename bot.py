...
API OpenLigaDB & ESPN avec modèles statistiques avancés
"""
...
class Config:
    ...
    # Toutes les ligues OpenLigaDB supportées
    OPENLIGADB_LEAGUES = {
        ...
        # Ajouter d'autres ligues OpenLigaDB ici
    }
    
    # Ligues ESPN supportées (format: sport/league)
    ESPN_LEAGUES = {
        # Soccer
    ...
    def init_db(self):
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS predictions (
                ...
                match_id TEXT UNIQUE, -- Change to TEXT to accommodate both IDs
                league TEXT,
                home_team TEXT,
                away_team TEXT,
                match_date TEXT,
    ...
    def save_prediction(self, pred):
        ...
        try:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                str(pred.get('match_id')), # Assurez-vous que c'est une chaîne
                pred.get('league'),
                pred.get('home_team'),
                pred.get('away_team'),
                pred.get('date'),
    ...
    def mark_sent(self, match_id):
        ...
        try:
            ...
            cursor.execute('''
                ...
                VALUES (DATE('now'), ?)
            ''', (str(match_id),)) # Assurez-vous que c'est une chaîne
            self.conn.commit()
            return True
        except Exception as e:
    ...
    def save_team_stats(self, stats):
        ...
        try:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                str(stats['team_id']), # Assurez-vous que c'est une chaîne
                stats['team_name'],
                stats['season'],
                stats['league'],
...
class OpenLigaDBCollector:
    ...
    async def fetch_matches_today(self):
        ...
        # Filtrer les ligues OpenLigaDB
        openligadb_leagues_info = {k: v for k, v in Config.OPENLIGADB_LEAGUES.items() if v.get("sport") == "soccer"} # Pour l'instant, on se concentre sur le soccer
                        ...
                        if isinstance(data, list):
                            ...
                            if matches_for_today:
                                ...
                                for match in matches_for_today:
                                    ...
                                    match['source'] = 'openligadb' # Marquer la source
...
class ESPNCollector:
    ...
    async def fetch_matches_today(self):
        ...
        for league_code, league_info in espn_leagues_info.items():
            try:
                ...
                # L'API ESPN pour le scoreboard peut prendre des paramètres de date
                ...
                async with self.session.get(url, params=params, timeout=Config.API_TIMEOUT) as response:
                    if response.status == 200:
                        ...
                        if matches_for_today:
                            logger.info(f"✅ {league_info['name']}: {len(matches_for_today)} match(s) aujourd'hui")
                            for match in matches_for_today:
                                match['source'] = 'espn' # Marquer la source
                            all_matches.extend(matches_for_today)
                    elif response.status == 404:
    ...
    def _parse_scoreboard(self, data, league_name):
        ...
        for event in data['events']:
            try:
                ...
                # Extraire les scores finaux si le match est terminé
    ...
    async def fetch_team_history(self, team_id, team_name, league_code, limit=10):
        """Récupère l'historique d'une équipe ESPN (SIMULE car ESPN n'a pas d'historique direct)"""
        # Pour ESPN, il n'y a pas d'endpoint simple pour l'historique complet.
        # On va simuler l'historique à partir des données récentes du scoreboard
        # ou en utilisant des endpoints spécifiques à une équipe si disponibles.
        # Pour simplifier, on va générer un historique basique.
        logger.debug(f"🔍 Historique ESPN simulé pour {team_name} (ID: {team_id})")
        
        # Simuler un historique basé sur des stats ou des matchs récents si possibles
        # Pour l'instant, on retourne une liste vide ou une liste simulée
        # NOTE: Cela reste une limitation sans endpoint d'historique direct.
        # On pourrait potentiellement appeler le scoreboard avec des dates passées.
    ...
    def _simulate_history(self, team_name, limit):
        ...
        # Basé sur la réputation de l'équipe
        ...
        for i in range(limit):
            if is_top_team:
                if random.random() > 0.3: # 70% de gagner
                    team_score = random.randint(2, 4)
                    opp_score = random.randint(0, 1)
                    result = 'WIN'
                elif random.random() > 0.3: # 49% de match nul ou défaite, donc 30% de nul
                    ...
                    result = 'LOSS'
            else: # Equipe moyenne/faible
                if random.random() > 0.6: # 40% de gagner
                    team_score = random.randint(1, 2)
                    opp_score = random.randint(0, 1)
                    result = 'WIN'
                elif random.random() > 0.5: # 30% de match nul
...
import random # Ajouté pour la simulation
...
class MatchAnalyzer:
    ...
    def __init__(self):
        ...
        self.league_averages = {
            ...
            # Ajouter d'autres ligues selon besoin
        }
...
class FootballPredictionSystem:
    ...
    async def run_daily_analysis(self, test_mode=False):
        ...
        try:
            # 1. Collecte des matchs du jour (OpenLigaDB + ESPN)
            all_matches = []
...
def main():
    ...
    if '--help' in sys.argv:
        print("""
🚀 Football Prediction Bot avec 10 Modèles Statistiques (OpenLigaDB + ESPN)
        """)
        return
